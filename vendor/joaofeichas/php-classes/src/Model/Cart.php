...
class Cart extends Model
{
    ...
    public function getProductsTotal()
    {
        ...
        $results = Product::checkList($sql->select("
            SELECT SUM(vlprice) AS vlprice, SUM(vlwidth) AS vlwidth, SUM(vlheight) AS vlheight, SUM(vllength) AS vllength, SUM(vlweight) AS vlweight, COUNT(*) AS nrqtd
            FROM tb_products p
            INNER JOIN tb_cartsproducts cp ON cp.idproduct = p.idproduct
            WHERE cp.idcart = :idcart
            AND cp.dtremoved IS NULL
        ", [
            ':idcart' => $this->getidcart()
    ...
    public function setFreigth($zipcode)
    {
        ...
        if ($totals['nrqtd'] > 0) {
            ...
            $xml = simplexml_load_file("http://ws.correios.com.br/calculador/CalcPrecoPrazo.aspx?" . $qs);
